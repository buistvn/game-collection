...
import com.example.gamecollection.data.GameTrailer
import com.example.gamecollection.data.LoadingStatus
import kotlinx.coroutines.launch

class GameTrailerViewModel : ViewModel() {
    private val repository = GameRepository(RAWGService.create())

    private val _results = MutableLiveData<GameTrailer?>(null)
    val results: LiveData<GameTrailer?> = _results
    ...
    fun loadResults(
        ...
        viewModelScope.launch {
            ...
            val result = repository.loadGameTrailers(id, key)
